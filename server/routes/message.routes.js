...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const receiverId =
        exchange.OwnerID === req.userId ? exchange.SeekerID : exchange.OwnerID;
      ...
      // --- Real-time Socket.IO Message Emission ---
      const io = req.app.get("io") || global.io; // fallback
      const onlineUsers = req.app.get("onlineUsers") || global.onlineUsers;

      const receiverSocketId = onlineUsers.get(receiverId?.toString());
      if (receiverSocketId) {
        io.to(receiverSocketId).emit("newMessage", messages[0]); // send to direct socket
        console.log(`Sent real-time message to user ${receiverId}`);
      }

      // Also emit to exchange chat room (if frontend joined via join_room)
      io.to(`exchange_${exchange_id}`).emit("receive_message", messages[0]);
    } catch (error) {

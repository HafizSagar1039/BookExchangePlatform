  ...
  try {
    const query = `
      SELECT e.*, 
        ...
        requester.first_name as requester_name,requester.last_name as requester_lname, requester.email as requester_email,
        owner.first_name as owner_name,owner.last_name as owner_lname, owner.email as owner_email
      FROM bookexchange e
  ...
  try {
    const query = `
      SELECT 
        ...
        owner.first_name AS owner_name, owner.last_name, owner.email AS owner_email, owner.id AS owner_id
      FROM bookexchange e
      JOIN book b ON e.BookID = b.BookID
      JOIN users requester ON e.SeekerID = requester.id
      JOIN users owner ON b.OwnerID = owner.id
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [exchanges] = await db.query(
        `SELECT e.*, 
          b.Title, b.Author, b.BookImage,
          requester.first_name as requester_name, requester.last_name as requester_last_name,
      ...
      const exchange = exchanges[0];
      // fullName 
      const fullName = `${exchange.requester_name} ${exchange.requester_last_name}`;
      // 🔔 Real-time Notification
      await sendRealTimeNotification({
        toUserId: exchange.OwnerID,
        fromUserId: req.userId,
        bookId: exchange.BookID,
        message: `You have a new exchange request for "${exchange.Title}" from ${fullName}.`,
      });
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [exchanges] = await db.query(
        `SELECT e.*,e.SeekerID AS requester_id,b.Title, b.BookID, b.OwnerID as book_owner_id, b.book_condition
         FROM bookexchange e
         JOIN book b ON e.BookID = b.BookID
         WHERE e.id = ?`,
        [exchangeId]
      ...
      //   return res.status(400).json({
      //     success: false,
      //     message: `Exchange is already ${exchange.ExchangeStatus}`,
      //   });
      // }
      ...
      if (!exchange.requester_id) {
        ...
        await sendRealTimeNotification({
        toUserId: exchange.requester_id,
        fromUserId: exchange.OwnerID,
        bookId: exchange.BookID,
        message: note,
      });
